...
const listBulletinVehicleService = async (placa:string):Promise<Boletim[]> => {
    ...
    if(!findVehicle) {
        throw new AppError("veículo não existe",401)
    }
    const findBoletim = await boletimRepository.find({relations:{veiculo: true}, where:{ veiculo:{id:findVehicle.id}}})
